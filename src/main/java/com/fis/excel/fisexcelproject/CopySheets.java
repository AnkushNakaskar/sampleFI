...
public class CopySheets {
    ...
    public Workbook mergeExcelFilesWithoutSix(Workbook book, List<Sheet> sheets) throws IOException, InvalidFormatException {
        ...
        for (Sheet sheet : sheets) {
            Sheet inputSheet = book.getSheet(ExcelService.DESCRIPTION_BULK_UPLOAD_DOCUMENT_SHEET_NAME_SIX);
            if (inputSheet == null) {
                inputSheet = book.createSheet(ExcelService.DESCRIPTION_BULK_UPLOAD_DOCUMENT_SHEET_NAME_SIX);
            }
            copySheets(inputSheet, sheet);
        }
        return book;
    ...
    public Workbook mergeExcelFilesSheet(Workbook book, List<Sheet> sheets) throws IOException, InvalidFormatException {
        for (Sheet sheet : sheets) {
            Sheet inputSheet = book.getSheet(ExcelService.DESCRIPTION_BULK_UPLOAD_DOCUMENT_SHEET_NAME);
            if (inputSheet == null) {
                inputSheet = book.createSheet(ExcelService.DESCRIPTION_BULK_UPLOAD_DOCUMENT_SHEET_NAME);
            }
            copySheets(inputSheet, sheet);
        }
    ...
    public static void copySheets(Sheet newSheet, Sheet sheet, boolean copyStyle) {
        ...
        for (int i = sheet.getFirstRowNum(); i <= sheet.getLastRowNum(); i++) {
            ...
            Row destRow = newSheet.createRow(newSheet.getLastRowNum() + i);
            if (srcRow != null) {
